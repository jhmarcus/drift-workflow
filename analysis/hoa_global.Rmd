...
# add snp meta data
bim_df = read.table("../data/raw/NearEastPublic/HumanOriginsPublic2068_maf_geno_ldprune.bim", header=F)
...
mv_df = data.frame(var=1/flash_fit$fit$est.tau, mu=sqrt(d1) * delta_df$`1`, chrom=bim_df$chrom) %>%
        mutate(chrom_type=ifelse(chrom > 22, "Sex", "Autosome"))

p_mv = ggplot(mv_df, aes(x=mu, y=var, color=chrom_type)) + 
       geom_point() + 
...
Lets take a closer look at the drift factors to see if they are clustring in particular regions of the genome:

```{r flash-ld-factors-chrom-viz}
...
d = table1 %>% left_join(table2, on="ensembl_gene_id") %>% mutate(rsid=refsnp_id) %>% 
