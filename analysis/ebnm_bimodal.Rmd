...
I adapted code from `ashr` to extract the grids in [../code/ebnm_bimodal.R](https://github.com/jhmarcus/drift-workflow/blob/master/code/ebnm_bimodal.R). Below is a sanity check that my helper function extracts the "correct" grids:
...
It looks like the grids match the `ashr` implementation. I next wrote an additional helper function that estimates the prior $g$ using the the grid of "intervals" defined above (see `get_bimodal_g` in [../code/ebnm_bimodal.R](https://github.com/jhmarcus/drift-workflow/blob/master/code/ebnm_bimodal.R)):
...
We can see for this example only a few components get weight i.e. those with mass close to 0 and 1. Finally given this $g$ I wrote a `ebnm` function that computes the posterior with the fixed $g$ (see `ebnm_bimodal` in [../code/ebnm_bimodal.R](https://github.com/jhmarcus/drift-workflow/blob/master/code/ebnm_bimodal.R)):
