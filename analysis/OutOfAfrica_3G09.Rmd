...
Here I visualize population structure with simulated data from [OutOfAfrica_3G09](https://stdpopsim.readthedocs.io/en/latest/catalog.html#sec_catalog_homsap_models_outofafrica_3g09) scenario. See [https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1000695]([Figure 2.]) from Gutenkunst et al. 2009.
...
```{r}
Y <- t(as.matrix(read.table("../output/simulations/OutOfAfrica_3G09/rep1.txt", sep=" ")))
n <- nrow(Y)
maf <- colSums(Y) / (2*n)
...
Runs ALStructure with $K=3$ (note ALStructure is an alternating least squares algorithm for fitting the PSD model):
...
```{r}
flbf <- fl %>% flash.backfit() %>% flash.nullcheck(remove = TRUE)
plot_loadings(flbf$flash.fit$EF[[1]], labs) + ggsave("../test.png")
```
